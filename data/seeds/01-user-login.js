...
function postRecordsInSeed(knex, record) {
    return knex('patient-login').insert({
        phoneNumber: record.field.Phone,
        loginTime: 0,
        clientId: record.fields['Coaching master table']
    });
}
    ...
    axios
        ...
        .then(results => {
            ...
            for (let i = 0; i < results.data.records.length; i++) {
                userArray.push({
                    phoneNumber: results.data.records[i].field.Phone,
                    loginTime: 0,
                    clientId:
                        results.data.records[i].fields['Coaching master table']
                });
            }

            return knex('patient-login').insert(userArray);
        })
        .catch(err => {
            console.log(err);
        });
