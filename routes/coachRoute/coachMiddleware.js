...
function validateCoachName(req, res, next) {
  ...
  axios
    ...
    .catch(err => {
      res.status(400).json({ error: err, message: "validateCoachName" });
    });
}
              ...
              coachDb
                ...
                .then(res => {
                  ...
                  coachDb
                    .insertNewPatient({
                      userId: res.userId,
                      patientId: req.patientInfo[i].patientId,
                      patientName: req.patientInfo[i].patientName,
                      coachId: req.patientInfo[i].coachId
                    })
                    .then(res => {
                      console.log("patient added to patient table");
                    })
                    .catch(err => {
                      console.log("error from patient table", err);
                    });
                })
                .catch(err => {
                  console.log("unable to find user with phone number");
                });
            })
            .catch(err => {
